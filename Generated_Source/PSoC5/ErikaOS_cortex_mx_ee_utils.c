...
#if (Number_of_ISRs > 0) && defined isr_buttons__INTC_NUMBER
    ramVectorTable[CY_INT_IRQ_BASE + (isr_buttons__INTC_NUMBER & CY_INT_NUMBER_MASK)] = isr_buttons;
	//PF v2.5.3
    //NVIC_SET_PRI(isr_buttons__INTC_NUMBER, (ISR_MIN_PRI - 1));
    CyIntSetPriority(isr_buttons__INTC_NUMBER, ISR_SCALE(1));
    //\PF
	NVIC_INT_ENABLE(isr_buttons__INTC_NUMBER);
#elif Number_of_ISRs > 0
#error  The Interrupt isr_buttons could not be found. Make sure the name in the OS config exactly matches the name of the interrupt in the interrupt tab.
#endif	/* isr_buttons */

#if (Number_of_ISRs > 1) && defined isr_timer__INTC_NUMBER
    ramVectorTable[CY_INT_IRQ_BASE + (isr_timer__INTC_NUMBER & CY_INT_NUMBER_MASK)] = isr_timer;
	//PF v2.5.3
    //NVIC_SET_PRI(isr_timer__INTC_NUMBER, (ISR_MIN_PRI - 1));
    CyIntSetPriority(isr_timer__INTC_NUMBER, ISR_SCALE(1));
    //\PF
	NVIC_INT_ENABLE(isr_timer__INTC_NUMBER);
#elif Number_of_ISRs > 1
#error  The Interrupt isr_timer could not be found. Make sure the name in the OS config exactly matches the name of the interrupt in the interrupt tab.
#endif	/* isr_timer */

#if (Number_of_ISRs > 2) && defined isr_timer_arcadian__INTC_NUMBER
    ramVectorTable[CY_INT_IRQ_BASE + (isr_timer_arcadian__INTC_NUMBER & CY_INT_NUMBER_MASK)] = isr_timer_arcadian;
	//PF v2.5.3
    //NVIC_SET_PRI(isr_timer_arcadian__INTC_NUMBER, (ISR_MIN_PRI - 5));
    CyIntSetPriority(isr_timer_arcadian__INTC_NUMBER, ISR_SCALE(5));
    //\PF
	NVIC_INT_ENABLE(isr_timer_arcadian__INTC_NUMBER);
#elif Number_of_ISRs > 2
#error  The Interrupt isr_timer_arcadian could not be found. Make sure the name in the OS config exactly matches the name of the interrupt in the interrupt tab.
#endif	/* isr_timer_arcadian */
